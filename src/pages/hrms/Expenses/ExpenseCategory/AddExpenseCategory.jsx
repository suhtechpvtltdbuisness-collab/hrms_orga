...
const AddExpenseCategory = () => {
    ...
    const handleSave = () => {
        ...
        if (id) {
            const updatedCategories = categories.map(c => c.id.toString() === id ? { ...dataToSave, id: c.id } : c);
            ...
            localStorage.setItem('expenseCategories', JSON.stringify([...categories, newCategory]));
        }
    ...
    return (
        <div className="bg-white px-4 sm:px-6 md:px-6 py-6 mx-2 sm:mx-4 mt-4 mb-4 rounded-xl h-[calc(100vh-10rem)] flex flex-col font-inter" style={{ fontFamily: 'Inter, sans-serif' }}>
            ...
            <div className="border border-[#E0E0E0] rounded-xl p-4">
                <h2 className="text-[16px] font-semibold text-[#1E1E1E] mb-2" style={{ fontFamily: 'Nunito Sans, sans-serif' }}>Category Details</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {/* Category Name */}
                    <div>
                        <label className="block text-[14px] font-medium text-[#344054] mb-2" style={{ fontFamily: 'Inter, sans-serif' }}>Category Name</label>
                        <input 
                            type="text"
                    ...
                    {/* Linked Account */}
                    <div>
                         <label className="block text-[14px] font-medium text-[#344054] mb-2" style={{ fontFamily: 'Inter, sans-serif' }}>Linked Expense Account</label>
                         <FilterDropdown
                            label=""
                            placeholder="Select account"
                            options={['Equipment 6300', 'Travel 6200', 'Meals 6000', 'Other 6700', 'Rent 6400', 'Utilities 6500']}
                            value={formData.linkedAccount}
                            onChange={(val) => setFormData({...formData, linkedAccount: val})}
                    ...
                    {/* Monthly Budget */}
                    <div>
                        <label className="block text-[14px] font-medium text-[#344054] mb-2" style={{ fontFamily: 'Inter, sans-serif' }}>Monthly Budget</label>
                        <input 
                            type="text"
                            className="w-full border border-[#D0D5DD] rounded-lg px-4 py-2.5 text-[16px] text-[#1E1E1E] focus:outline-none focus:border-[#7D1EDB] placeholder-[#757575]"
                    ...
                    {/* Daily Limit */}
                    <div>
                        <label className="block text-[14px] font-medium text-[#344054] mb-2" style={{ fontFamily: 'Inter, sans-serif' }}>Daily Limit</label>
                        <input 
                            type="text"
                            className="w-full border border-[#D0D5DD] rounded-lg px-4 py-2.5 text-[16px] text-[#1E1E1E] focus:outline-none focus:border-[#7D1EDB] placeholder-[#757575]"
                            placeholder="Enter amount"
