...
export default function ProjectDetails() {
  ...
  const availablePeople = useMemo(() => people.filter((person) => !members.some((member) => String(member.userId || member.id) === String(person.id))), [people, members]);
  const completed = useMemo(() => tasks.filter((task) => task.status === 'COMPLETED').length, [tasks]);

  // The roster must never be empty, otherwise "Add member" has nothing to pick.
  ...
  const saveProject = async (event) => {
    ...
    try {
      ...
      await projectService.update(id, { ...projectForm, ownerId: projectForm.ownerId || undefined });
  ...
  const addMember = async () => {
    ...
    try {
      setMemberBusy(true);
      await projectService.addMember(id, memberUserId);
      setMemberUserId('');
      await load();
  ...
  return <div className="min-h-[calc(100vh-88px)] rounded-2xl bg-[#f7f7fa] p-4 sm:p-6">
    ...
    {usingDemo && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">Demo mode: the project API is not connected yet, so changes are saved in this browser only.</div>}
