...
const ChartOfAccounts = () => {
  ...
  useEffect(() => {
    const loadAccounts = async () => {
      ...
      setAccountData(result.success ? result.data || [] : []);
    };
    loadAccounts();
  }, []);

  const handleDelete = async (id) => {
    const result = await accountsService.deleteChartAccount(id);
    if (!result.success) {
      alert(result.message || "Failed to delete account");
      return;
    }
    setAccountData((current) => current.filter((acc) => acc.id !== id));
  };
            ...
            {type.accounts.length > 0 ? (
              type.accounts.map((account, accIndex) => (
                <div
                  ...
                  onClick={() =>
                    navigate("/hrms/chart-of-accounts/add", {
                      state: { account, viewMode: true },
                    })
                  }
                >
